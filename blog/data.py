...
class APost(object):
    ...
    def init_data(self):
        ...
        self.post[
            'pub_date_mixed'] = {'year': (str(self.article.publish_date.year)),
                                 'day':
                                 str(self.article.publish_date.day),
        ...
        # use all get all many-to-many ojbects,in foreign key relatives.
        self.post['tags'] = self.article.tags.all()
                                                    # don't need to call all()
                                                    # function,just access
                                                    # it(for example
                                                    # threadtypeid) .
...
class HeaderMenu(object):
    ...
    def __init__(self):
        self.ttypes_display = []
        pthread_types = ThreadTypes.objects.filter(
            parent_id=0).order_by(
            "display_order")
        for pthread_type in pthread_types:
            if pthread_type.name:
                link = settings.BLOG_THREADTYPE_URL + \
                    '/' + pthread_type.name + '/'
                thread_type = {'parent': pthread_type,
                               'link': link,
                               'children': []}
                cthread_types = ThreadTypes.objects.filter(
                    parent_id=pthread_type.id).order_by(
                    "display_order")
                for cthread_type in cthread_types:
                    clink = settings.BLOG_THREADTYPE_URL + \
                        '/' + cthread_type.name
...
class ArchivesIndex(object):
    ...
    def by_month(self):
        ...
        self.bymonth_dict = {}
        # return a year_mounth list order by month
        months = Posts.objects.dates("publish_date", "month")
        # return a year list(datetime object) order by year
        years = Posts.objects.dates("publish_date", "year")
        self.yearlist = []
    ...
    def has_nextyear(self, year):
        yearlist = sorted([ykey for ykey in self.bymonth_dict])
        ylen = len(yearlist)
    ...
    def has_next(self):
        ...
        if self.year and self.month:
            ...
            if next_month:
                ...
                if next_year:
                    self.next = {'year': next_year, 'month':
                                 self.bymonth_dict.get(next_year)[0]}
                else:
                    self.next = None
        else:
            self.next = None
    ...
    def has_pre(self):
        ...
        if self.year and self.month:
            ...
            if pre_month:
                ...
                if pre_year:
                    self.pre = {'year': pre_year, 'month':
                                self.bymonth_dict.get(pre_year)[-1]}
                else:
                    self.pre = None
        else:
            self.next = None
...
class PostsGetter(object):
    ...
    def get_lastest(self, num=None, displayall=False):
        ...
        if not displayall:
            posts = Posts.objects.filter(
                status=2).order_by("-publish_date")[:num]
        else:
    ...
    def get_hotest(self, num=None, default_days=None, displayall=False):
        ...
        if not displayall:
            ...
            posts = Posts.objects.filter(
                publish_date__range=(date_from,
                                     date_to)).order_by("-comment_count")[:num]
        if posts:
...
def get_summarys_bytag(tagname, displayall=False):
    ...
    posts = Posts.objects.filter(
        status__id=2,
        tags__tagname=tagname).order_by(
        "-publish_date")
    return get_summarys(posts)
